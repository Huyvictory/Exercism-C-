...
public class SecurityPassMaker
{
    ...
    {
        var typeTeamMember = support.GetType();

        var title = support is Staff ? support.Title : "Too Important for a Security Pass";

        var securityPersonnel =
            typeTeamMember == typeof(Security)
                ? "Priority Personnel"
                : "";

        return $"{title} {securityPersonnel}".Trim();
    }
}
